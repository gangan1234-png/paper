...
class OptimizedBERTopicAnalyzer:
    def __init__(self, csv_path, output_dir="BERTopic_Analysis_Radical"):
        ...
        self.color_palette = {t: '#B0B0B0' for t in range(-1, 20)}
    ...
    def plot_static_clusters(self, topic_model):
        ...
        try:
            ...
            umap_2d = UMAP(n_neighbors=2, n_components=2, min_dist=0.05, metric='cosine',
                           random_state=42).fit_transform(embeddings)
            bubble_colors = [self.color_palette.get(t, '#B0B0B0') for t in topics]

            plt.figure(figsize=(10, 8), dpi=300)
            bubble_sizes = freq * 35 + 600

            plt.scatter(umap_2d[:, 0], umap_2d[:, 1], s=bubble_sizes, c=bubble_colors, alpha=0.7, edgecolors='white',
                        linewidth=1.5)
            ...
            plt.title('激进组空间感知聚类 (n_neighbors=2)', fontsize=16, pad=20)
            plt.xticks([])
            plt.yticks([])
            ...
            # 【修改】保存为矢量图 SVG 格式
            save_path = f"{self.output_dir}/static_cluster_map_radical.svg"
    ...
    def visualize_results(self, topic_model):
        try:
            ...
            fig_hierarchy.update_traces(marker=dict(color='#B0B0B0'))
